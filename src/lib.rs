...
/// Find byte in a bytes.
#[inline]
pub fn find_sse(value: &[u8], byte: u8) -> Option<usize> {
    unsafe { find_sse_raw(value.as_ptr(), value.as_ptr().add(value.len()), byte) }
}

#[target_feature(enable = "sse4.2")]
fn find_sse_raw(start: *const u8, end: *const u8, byte: u8) -> Option<usize> {
